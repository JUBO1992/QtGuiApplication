...
struct QPoint3D
{
	...
	QPoint3D(double X, double Y, double Z) :x(X), y(Y), z(Z){}
};

class CoordTransBase
{
public:
	CoordTransBase(){}
	virtual ~CoordTransBase(){}

public:
	/*!	@brief 坐标正算：1.从地理坐标到投影坐标；2.从源坐标系到目标坐标系
	*	@param[in] ix 输入x坐标（东方向），若为地理坐标则要求输入十进制度
	*	@param[in] iy 输入y坐标（北方向），若为地理坐标则要求输入十进制度
	*	@param[out] ox 输出x坐标（东方向）
	*	@param[out] oy 输出y坐标（北方向）
	*	@return 返回是否转换成功
	*/
	virtual bool forwardCoordTrans(double ix, double iy, double& ox, double& oy) const = 0;
	/*!	@brief 坐标反算：1.从投影坐标到地理坐标；2.从目标坐标系到源坐标系
	*	@param[in] ix 输入x坐标（东方向）
	*	@param[in] iy 输入y坐标（北方向）
	*	@param[out] ox 输出x坐标（东方向），若为地理坐标则输出十进制度
	*	@param[out] oy 输出y坐标（北方向），若为地理坐标则输出十进制度
	*	@return 返回是否转换成功
	*/
	virtual bool inverseCoordTrans(double ix, double iy, double& ox, double& oy) const = 0;
};

//!地理坐标和投影坐标相互转换
class GeocsProjcsTrans :public CoordTransBase
{
public:
	GeocsProjcsTrans(ProjDefBase* pProjDef);
	virtual ~GeocsProjcsTrans();

	/*!	@brief 坐标正算：从地理坐标到投影坐标
	*	@param[in] ix 输入x坐标（东方向），要求输入十进制度
	*	@param[in] iy 输入y坐标（北方向），要求输入十进制度
	*	@param[out] ox 输出x坐标（东方向）
	*	@param[out] oy 输出y坐标（北方向）
	*	@return 返回是否转换成功
	*/
	virtual bool forwardCoordTrans(double ix, double iy, double& ox, double& oy)const override;
	/*!	@brief 坐标反算：从投影坐标到地理坐标
	*	@param[in] ix 输入x坐标（东方向）
	*	@param[in] iy 输入y坐标（北方向）
	*	@param[out] ox 输出x坐标（东方向），输出十进制度
	*	@param[out] oy 输出y坐标（北方向），输出十进制度
	*	@return 返回是否转换成功
	*/
	virtual bool inverseCoordTrans(double ix, double iy, double& ox, double& oy)const override;
...
public:
	...
	*/
	...
	//利用正反算分步转换
	bool transform_1(double ix, double iy, double& ox, double& oy) const;//!<从源到目标
	bool inverse_1(double ix, double iy, double& ox, double& oy) const;//!<从目标到源
	//利用pj_transform一步转换
	bool transform_2(double ix, double iy, double& ox, double& oy) const;//!<从源到目标
	bool inverse_2(double ix, double iy, double& ox, double& oy) const;//!<从目标到源
...
/*!	@brief 坐标转换类
*	1.实现四参数平面坐标转换和七参数空间坐标转换
*	2.四参数平面坐标转换：平面坐标基本转换
*	3.七参数空间坐标转换：进行不同基准面的坐标转换的中间步骤
*	4.不同基准面坐标转换流程：投影->大地->空间-七参数转换-空间->大地->投影
*	5.
*/
