...
{
public:
	ProjDefBase() :_datumType(CGCS2000), _ellip(_datumType), _prjType(NoneProj){}
	virtual~ProjDefBase(){}

	ProjType GetProjType()const { return _prjType; }
...
//!无投影定义-地理坐标
class NoneProjDef :public ProjDefBase
{
public:
	NoneProjDef(){}
	virtual~NoneProjDef(){}
...
class MercProjDef :public ProjDefBase
{
public:
	MercProjDef() :_lon_0(0), _lat_ts(0), _x_0(0), _y_0(0){ _prjType = MercProj; }
	virtual~MercProjDef(){}
	...
	virtual QString ProjStr() const override
	{
		return QString("+proj=merc +a=%1 +f=%2 +lon_0=%3 +lat_ts=%4 +x_0=%6 +y_0=%7 +units=m +no_defs")
			.arg(_ellip._a, 0, 'f', 20).arg(_ellip._f, 0, 'f', 20)
			.arg(_lon_0, 0, 'f', 20).arg(_lat_ts, 0, 'f', 20)
			.arg(_x_0, 0, 'f', 20).arg(_y_0, 0, 'f', 20);
	}

private:
	double _lon_0;		//投影中央经线
	double _lat_ts;		//基准纬线
	double _x_0;		//东偏
	double _y_0;		//北偏
};
...
class TMercProjDef :public ProjDefBase
{
public:
	TMercProjDef() :_lon_0(0), _lat_0(0), _k_0(1), _x_0(0), _y_0(0){ _prjType = TMercProj; }
	...
	virtual QString ProjStr() const override
	{
		return QString("+proj=tmerc +a=%1 +f=%2 +lon_0=%3 +lat_0=%4 +k_0=%5 +x_0=%6 +y_0=%7 +units=m +no_defs")
			.arg(_ellip._a, 0, 'f', 20).arg(_ellip._f, 0, 'f', 20)
			.arg(_lon_0, 0, 'f', 20).arg(_lat_0, 0, 'f', 20)
			.arg(_k_0, 0, 'f', 20)
			.arg(_x_0, 0, 'f', 20).arg(_y_0, 0, 'f', 20);
	}
...
private:
	double _lon_0;		//投影中央经线
	double _lat_0;		//投影中央纬线
	double _k_0;		//比例因子（UTM:0.9996,Guass-Kruger:1.0）
	double _x_0;		//东偏
	double _y_0;		//北偏
};

//!Web墨卡托投影定义（网络地图使用的投影）
class WebMercProjDef :public ProjDefBase
{
public:
	WebMercProjDef() :_lon_0(0), _x_0(0), _y_0(0){ _prjType = WebMercProj; }
